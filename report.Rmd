...
How can our model be used - introduction? 
A summary of the prediction result and inference results. 
Future directions 
Cost of living adjustment 


As we mentioned in our introduction, through our regression analysis, we wanted to investigate what individual characteristics of second-generation school-aged immigrants in the United States predict income roughly a decade or more in the future. Understanding second-generation immigrants and predicting their future income can be convenient for policymaking since these insights may help us allocate our limited resources more efficiently. 

Using our dataset and LASSO, we could make a model, but unfortunately, it turns out that the model was useless for both prediction and causal inferences. Since it violated many assumptions of linear regression, the power of the prediction was weak enough to be almost meaningless. Also, we concluded that our model was not suitable for causal inference analysis since we don’t have the domain knowledge to identify all the confounders. 

Besides all these issues, our analysis concludes that using a null model for the prediction will be the best, and self-esteem is the most consistent factor that positively influences income. In other words, when it comes to policymaking, it is safe to assume that all second-generation students will have the mean income regardless of individual characteristics. Making a new government policy that can improve students' self-esteem might help them have more income in the future. Honestly, our analysis gave us a very dull and somewhat depressing conclusion, but since our research had lots of limitations, it is more likely that our conclusion was wrong. We believe and hope that our reality is much brighter than what we found in this analysis.

Our group understands that our research objective is more closely related to causal inference than a prediction since we are interested in which variable impacts income most. However, we concluded that it would be difficult to make causal inferences because we don’t have the domain knowledge to identify all the confounders; we do not even know if this data set has all the variables we need for the analysis. Because of this limitation of our dataset, we will primarily focus on predicting income. We will also make a causal inference analysis, but our analysis can be very off because of the limitations of the dataset. 

# Reference

[1] https://www.pewresearch.org/social-trends/2013/02/07/second-generation-americans/

# Appendix

```{r prediction-plot2, fig.width=6, fig.height=3}
plot(df_test.pred$s1, test[,1] - df_test.pred$s1, xlab = "Prediction", ylab = "Residual")
abline(h = 0)
```
